...
*     .                   wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag )
      SUBROUTINE ZGEXPV_CACHE( nspin, nTerm, coeff_lst, 
...
*----------------------------------------------------------------------|
     ...
     .                 vnorm, avnorm, hj1j, hump, SQR1
      ...
      ! for debug; profile
      INTEGER, PARAMETER  ::  ttn = 4   ! number of profiles
...
!      ! for debug;
!      COMPLEX*16, ALLOCATABLE ::  wsp2(:)
!      ALLOCATE(wsp2(ham_dim))
...
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 01 = ",cuda_status ! for debug;
      cuda_status = cuda_malloc(dev_v, n * szpx)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 02 = ",cuda_status ! for debug;
      
      ! hamvec_cuda3;
      cuda_status = cuda_malloc(dev_w_med, ham_dim * szpx)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 03 = ",cuda_status ! for debug;
      cuda_status = cuda_malloc(dev_mat_i_lst, mat_idx * szpx)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 04 = ",cuda_status ! for debug;
      cuda_status = cuda_malloc(dev_coeff_lst_zplx, nTerm * szpx)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 05 = ",cuda_status ! for debug;
      
      cuda_status = cuda_memcpy_fort2c_real(dev_w, v, ham_dim * szpx, 1)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 06 = ",cuda_status ! for debug;
      cuda_status = cuda_memcpy_fort2c_real(dev_mat_i_lst, mat_i_lst, 
     .  mat_idx * szpx, 1)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 07 = ",cuda_status ! for debug;
      cuda_status = cuda_memcpy_fort2c_real( dev_coeff_lst_zplx, 
     .  coeff_lst_zplx, nTerm * szpx, 1)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 08 = ",cuda_status ! for debug;
      cuda_status = cuda_memcpy_fort2c_real(dev_w_med, v, 
     .  ham_dim * szpx, 1)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 09 = ",cuda_status ! for debug;
      cuda_status = cuda_memcpy_fort2c_real(dev_v, v, 
     .  ham_dim * szpx, 1)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 10 = ",cuda_status ! for debug;
      ...
      END DO
      bj = DZNRM2( ham_dim, qj, 1 )
      CALL ZDSCAL( ham_dim, 1.0D0/bj, qj, 1 )
      bjm = 0.0D0
      DO j = 1, k
!        CALL hamvec_func3( nspin, nTerm, coeff_lst_zplx, nbody_lst, 
!     .  pos_i_idx, pos_i_lst, dim_i_lst, mat_i_idx, mat_i_lst, ham_dim, 
!     .  nspin_dim, nspin_m_lst, nspin_n_lst, 
!     .  qj, w, w_med )
        
        cuda_status = cuda_memcpy_fort2c_real( dev_v, qj, 
     .    ham_dim * szpx, 1 )
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 11 = ",cuda_status ! for debug;
        CALL hamvec_cuda3( cublas_handle, nspin, nTerm, coeff_lst_zplx, 
     .    nbody_lst, pos_i_idx, pos_i_lst, dim_i_lst, mat_i_idx, 
     .    dev_mat_i_lst, ham_dim, nspin_dim, nspin_m_lst, nspin_n_lst, 
     .    dev_v, dev_w, dev_w_med, dev_coeff_lst_zplx,
     .    maxThreadsPerBlock, maxGridSize )
        cuda_status = cuda_memcpy_c2fort_real( w, dev_w, 
     .    n * szpx, 2)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 12 = ",cuda_status ! for debug;
!        cuda_status = cuda_memcpy_c2fort_real( wsp2, dev_w, 
!     .    n * szpx, 2)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 12 = ",cuda_status ! for debug;
!        CALL ZDOTC( aj, ham_dim, wsp2, 1, w, 1 )
!        WRITE(*,*) "hamvec 01 err = ", ABS(1.0D0 - aj 
!     .  /DZNRM2(ham_dim,wsp2,1)**2 )
        
        CALL ZDOTC( aj, ham_dim, qj, 1, w, 1 )
        tk( (j-1)*k+j ) = DREAL(aj);
        CALL ZAXPY( ham_dim, -aj, qj, 1, w, 1 )
        CALL ZAXPY( ham_dim, -bjm, qjm, 1, w, 1 )
        bj = DZNRM2( ham_dim, w, 1 )
        IF ( j .EQ. k ) EXIT
        tk( (j-1)*k+j+1) = bj;
        tk( j*k+j ) = bj;
        IF ( bj .LT. tol ) THEN
          EXIT
        ELSE
          CALL ZCOPY( ham_dim, qj, 1, qjm, 1 )
          CALL ZCOPY( ham_dim, w, 1, qj, 1 )
          CALL ZDSCAL( ham_dim, 1.0D0/bj, qj, 1 )
          bjm = bj
        END IF
      END DO
...
*
 ...
 100  if ( t_now.ge.t_out ) goto 500
      ...
      ! for debug;
!      WRITE(*,*) 'p1 = ', p1 ! for debug;
!      WRITE(*,"(A,I4,A,I8,A,E10.4)") "nspin = ", nspin, ", nstep = ", 
!     .  nstep, ", t_step = ", t_step
...
!     .  wsp(j1v-n), wsp(j1v), w_med )
        ...
        cuda_status = cuda_memcpy_fort2c_real( dev_v, wsp(j1v-n), 
     .    ham_dim * szpx, 1 )
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 13 = ",cuda_status ! for debug;
        CALL hamvec_cuda3( cublas_handle, nspin, nTerm, coeff_lst_zplx, 
     .    nbody_lst, pos_i_idx, pos_i_lst, dim_i_lst, mat_i_idx, 
     .    dev_mat_i_lst, ham_dim, nspin_dim, nspin_m_lst, nspin_n_lst, 
     .    dev_v, dev_w, dev_w_med, dev_coeff_lst_zplx,
     .    maxThreadsPerBlock, maxGridSize )
        cuda_status = cuda_memcpy_c2fort_real( wsp(j1v), dev_w, 
     .    n * szpx, 2)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 14 = ",cuda_status ! for debug;
!        cuda_status = cuda_memcpy_c2fort_real( wsp2, dev_w, 
!     .    n * szpx, 2)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 14 = ",cuda_status ! for debug;         
!        CALL ZDOTC( aj, ham_dim, wsp2, 1, wsp(j1v), 1 )
!        WRITE(*,*) "hamvec 02 err = ",  ABS(1.0D0 - aj 
!     .  /DZNRM2(ham_dim,wsp2,1)**2 )
...
!     .  wsp(j1v-n), wsp(j1v), w_med )
        ...
        cuda_status = cuda_memcpy_fort2c_real( dev_v, wsp(j1v-n), 
     .    ham_dim * szpx, 1 )
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 15 = ",cuda_status ! for debug;
        CALL hamvec_cuda3( cublas_handle, nspin, nTerm, coeff_lst_zplx, 
     .    nbody_lst, pos_i_idx, pos_i_lst, dim_i_lst, mat_i_idx, 
     .    dev_mat_i_lst, ham_dim, nspin_dim, nspin_m_lst, nspin_n_lst, 
     .    dev_v, dev_w, dev_w_med, dev_coeff_lst_zplx,
     .    maxThreadsPerBlock, maxGridSize )
        cuda_status = cuda_memcpy_c2fort_real( wsp(j1v), dev_w, 
     .    n * szpx, 2)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 16 = ",cuda_status ! for debug;
!        cuda_status = cuda_memcpy_c2fort_real( wsp2, dev_w, 
!     .    n * szpx, 2)
!      IF (cuda_status .NE. 0) WRITE(*,*) "cuda_status 16 = ",cuda_status ! for debug;
!        CALL ZDOTC( aj, ham_dim, wsp2, 1, wsp(j1v), 1 )
!        WRITE(*,*) "hamvec 03 err = ",  ABS(1.0D0 - aj 
!     .  /DZNRM2(ham_dim,wsp2,1)**2 )
...
*
 ...
 500  continue
      ...
      CALL ZCOPY( n, w, 1, w_seq((tt-1)*n+1), 1 )
      
      END DO ta_output
      
      ! cuda, deallocate memory
      CALL cuda_free(dev_w)
      CALL cuda_free(dev_v)
      CALL cuda_free(dev_w_med)
      CALL cuda_free(dev_mat_i_lst)
      CALL cuda_free(dev_coeff_lst_zplx)
