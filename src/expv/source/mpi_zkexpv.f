...
!     .                   wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag )
      ...
      SUBROUTINE MPI_ZKEXPV(n, m, v, w, tol, anorm, 
     .  wsp, lwsp, iwsp, liwsp, itrace, iflag, 
     .  nspin, coeff_lst, nterm, tlst, tn, op_coeff_lst, n_op, oplst, 
     .  reqs, status, cache, nchunk)
      implicit none
      
      INCLUDE "mpif.h"
      
      INTEGER           nspin, nterm, tn, nchunk, rank, ierr, n_op
      DOUBLE PRECISION  tlst(tn), coeff_lst(nterm),op_coeff(nterm)
      DOUBLE PRECISION  op_coeff_lst(nterm * n_op)
...
*
     ...
     .  MPI_COMM_WORLD, ierr)
      beta = cr
      hump = MAX( hump, beta )
!      WRITE(*,*) "t_now = ", t_now, ", beta = ", beta! for debug;
...
!$OMP PARALLEL DO
        ...
        enddo
        do i=1,2**(nspin/2)
           wsp ( n + i + ((i-1) * (2**(nspin/2))) ) = ONE
        enddo
        do i=1,nterm
            op_coeff(i) = op_coeff_lst(i + (j-1)*nterm)
        enddo
        CALL mpi_hamvec(nspin, op_coeff, w, wsp(1),
     .       reqs,status, cache, nchunk)
        op = ZDOTC( n, wsp(n + 1), 1, wsp(1),1 )
