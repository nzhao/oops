*----------------------------------------------------------------------|
      subroutine ZGCOOVEXPV( n, m, t, v, w, tol, anorm, non_zeros
...
*     Test zgcoovexpv
      ...
      integer nzmax
      parameter(nzmax = 838860800)
      integer nz, seed, non_zeros
      integer ia(nzmax), ja(nzmax)
      double precision a(nzmax)
      common /CMAT/ a, ia, ja, nz
      integer flag_1
      seed = 5
      nz = non_zeros
      do flag_1 = 1, nz
         ia(flag_1) = 1 
         ja(flag_1) = 1 
         s1 = 10.0d0*DLARAN(seed) - 5.0d0
         s2 = 10.0d0*DLARAN(seed) - 5.0d0
         a(flag_1) = DCMPLX(s1, s2)
      enddo
      do flag_1 = 1, n
         wsp(flag_1) = ZERO
      enddo
      do flag_1 = 1, nz
         wsp(ia(flag_1)) = wsp(ia(flag_1)) +ABS( a(flag_1) )
      enddo
      anorm = wsp(1)
      do flag_1 = 1, n
        if( anorm.lt.DBLE(wsp(flag_1)) ) anorm = wsp(flag_1)
      enddo
*
