...
*     .                   wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag )
      SUBROUTINE KRYLOV_ZGEXPV( n, a, v, tn, ta, m, tol,
...
!!      for debug;
!      WRITE(*,*) ''
!      WRITE(*,*) 'in krylov_zgexpv:'
!      WRITE(*,*) 'SIZEOF(integer) = ', SIZEOF(i)
!      WRITE(*,*) 'SIZEOF(double)  = ', SIZEOF(anorm)
!      WRITE(*,*) 'SIZEOF(zomplex) = ', SIZEOF(hij)
!      WRITE(*,*) 'n     = ', n
!      WRITE(*,*) 'm     = ', m
!      WRITE(*,"(A, Z16)") ' a     =', LOC(a)
!      WRITE(*,*) 'tn    = ', tn
!      WRITE(*,"(A, Z16)") ' ta    =', LOC(ta)
!      WRITE(*,*) 'tol   = ', tol
!      WRITE(*,*) 'anorm = ', anorm
!      WRITE(*,"(A, Z16)") ' wsp   =', LOC(wsp)
!      WRITE(*,*) 'lwsp  = ', lwsp
!      WRITE(*,"(A, Z16)") ' iwsp  =', LOC(iwsp)
!      WRITE(*,*) 'liwsp = ', liwsp
!      WRITE(*,*) 'itrace= ', itrace
!      WRITE(*,*) 'iflag = ', iflag
!      WRITE(*,"(A, Z16)") ' w_seq  =', LOC(w_seq)
!      WRITE(*,*) 'w_seq_len = ', w_seq_len
!      WRITE(*,*) 'nz    = ', nz
...
!     .  qj, w, w_med )
        CALL ZGEMV('N', n, n, ONE, a, n, qj, 1, ZERO, w, 1)
...
!      ! for debug;
!      CALL FDATE(date)
!      WRITE(*,*) "Current time is ", date
!      WRITE(*,"(A,I4,A,I8,A,E10.4)") "  nspin = ", nspin, ", nstep = ", 
!     .  nstep, ", t_step = ", t_step
!      WRITE(*,"(A,E10.4,A,E10.4,A,E10.4)") "  t_now = ", t_now,
!     .  ", t_out = ", t_out, ", t_final = ", ta(tn)
*
*---  Arnoldi loop ...
*
      j1v = iv + n
...
!     .  wsp(j1v-n), wsp(j1v), w_med )
      CALL ZGEMV('N', n, n, ONE, a, n, wsp(j1v-n), 1, ZERO, wsp(j1v), 1)
         do i = 1,j
...
*
 ...
 500  continue
      ...
      END SUBROUTINE KRYLOV_ZGEXPV
*----------------------------------------------------------------------|

*----------------------------------------------------------------------|
*	 subroutine ZGEXPV( n, m, t, v, w, tol, anorm,
*     .                   wsp,lwsp, iwsp,liwsp, matvec, itrace,iflag )
      SUBROUTINE KRYLOV_ZCOOEXPV( n, nz, ia, ja, a, v, tn, ta, m, tol,
     .  anorm, wsp, lwsp, iwsp, liwsp, itrace, iflag, w_seq, w_seq_len )
...
*
 ...
 500  continue
      ...
      END SUBROUTINE KRYLOV_ZCOOEXPV
*----------------------------------------------------------------------|
